...
namespace graphics
{
	...
	// ステート記述用ヘルパー
    D3D12_DEPTH_STENCIL_DESC ModelPipeline::MakeDepthStencilDesc()
    {
    ...
    D3D12_BLEND_DESC ModelPipeline::MakeBlendDesc()
    {
    ...
    D3D12_RASTERIZER_DESC ModelPipeline::MakeRasterizerDesc()
    {
        D3D12_RASTERIZER_DESC desc = {};
        desc.FillMode = D3D12_FILL_MODE_SOLID;
        desc.CullMode = D3D12_CULL_MODE_BACK;
        desc.FrontCounterClockwise = FALSE;    // 左手系 (DirectX 標準)
        desc.DepthBias = 0;
    ...
    // ルートシグネチャ
    bool ModelPipeline::CreateRootSignature(ID3D12Device* device)
    {
        // t0:instancebuffer
        ...
        // t2:diffusebuffer
        CD3DX12_DESCRIPTOR_RANGE1 rangeDiffuse;
        rangeDiffuse.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 2);

        // t3:camerabuffer
        CD3DX12_DESCRIPTOR_RANGE1 rangeCamera;
        rangeCamera.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 3);

        // t4:normaltexture
        CD3DX12_DESCRIPTOR_RANGE1 rangeNormal;
        rangeNormal.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 4);
        ...
        params[SLOT_DIFFUSE_TEX].InitAsDescriptorTable(1, &rangeDiffuse, D3D12_SHADER_VISIBILITY_PIXEL);
        params[SLOT_CAMERA_BUFFER].InitAsDescriptorTable(1, &rangeCamera, D3D12_SHADER_VISIBILITY_VERTEX);
        ...
        if (FAILED(hr))
        {
            DEBUG_LOG(sys::eLogLevel::Fatal, "ModelPipeline: Failed to serialize root signature.");
            return false;
        }
        ...
        if (FAILED(hr))
        {
            DEBUG_LOG(sys::eLogLevel::Fatal, "ModelPipeline: Failed to create root signature.");
            return false;
        }
    ...
    bool ModelPipeline::CreatePipeline(ID3D12Device* device, ShaderManager& shaderManager)
    {
        auto VS = shaderManager.GetShader(
            ASSET_PATH("/Engine/Assets/Shader/Model/VS_Model.hlsl").string(), "main", "vs_6_0");
        auto PS = shaderManager.GetShader(
            ASSET_PATH("/Engine/Assets/Shader/Model/PS_Model.hlsl").string(), "main", "ps_6_0");

        if (!VS || !PS)
        {
            DEBUG_LOG(sys::eLogLevel::Fatal, "ModelPipeline: Failed to load shaders.");
            return false;
        }

        // ModelVertex と 1:1 対応した InputLayout
        // BinFmt::Vertex の各フィールドのオフセットに一致させる
        ...
        if (FAILED(hr))
        {
            DEBUG_LOG(sys::eLogLevel::Fatal, "ModelPipeline: Failed to create PSO.");
            return false;
        }
        mPipelineState->SetName(L"ModelPipeline");
        return true;
    }

    bool ModelPipeline::Create(DX12Device& device, ShaderManager& shaderManager)
    {
        ID3D12Device* d3d = device.GetDevice();
        if (!CreateRootSignature(d3d)) return false;
        if (!CreatePipeline(d3d, shaderManager)) return false;

        DEBUG_LOG(sys::eLogLevel::Log, "ModelPipeline: Created successfully.");
        return true;
    }
