...
namespace ecs
{
    ...
    enum class eWeaponType
    {
        ...
        VoidBeam,   // 貫通レーザー型：狙い不要で自動的に、最も近い敵の方向へ直線状の範囲を貫通させる
        BoneSpear,  // 貫通弾型：狙い不要で自動的に、最も近い敵へ直進する貫通弾を発射する
        Cleave,     // 近接扇状型：狙い不要で自動的に、狙い方向の扇状範囲内の敵をなぎ払いノックバックさせる
        FlickerStrike, // ワープ連撃型：手動(狙い指定)。パワーチャージを全消費し、近くの敵へ次々ワープ攻撃する
    };
