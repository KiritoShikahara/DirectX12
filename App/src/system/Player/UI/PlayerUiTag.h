...
namespace ecs
{
	...
	/// <summary>
	/// 必殺技ゲージのスプライトを識別するタグ。PlayerUltimateGaugeSystemがFillAmountを
	/// 撃破数の充填率で駆動する。HPバーと同じ画像を左右反転・青色で右下に配置したもの。
	/// </summary>
	...
	/// <summary>
	/// 所持武器アイコンバーの1スロットを識別するタグ。SlotIndexはWeaponInventoryComponent::Weapons
	/// のインデックスに対応し、Icon/Overlay/Text/LevelTextの4エンティティが同じSlotIndexを共有する。
	/// </summary>
	struct WeaponIconSlotTag
	{
		...
		/// <summary>
		/// Text/LevelText用、水平中央揃えの基準X座標。TextComponent::Xは幅に応じて毎フレーム
		/// 書き換わるため、基準値は別途ここに持たないと位置がズレ続ける。
		/// </summary>
		float CenterX = 0.0f;
	};
}
