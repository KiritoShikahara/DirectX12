...
## 2026-07-17 エフェクトが一瞬四角形ポリゴンになる問題への対策（仮説ベース、未確定）
- 内容: FrostOrbの周回オーブ・IceSpike等、`ecs::EffectComponent::IsLoop=true`で
  「素材自体の再生時間より長く表示し続けたい」効果は、`EffekseerManager::Update()`が
  毎フレーム`Effect.IsPlaying()`を監視し、falseになった(=素材の1回分の再生が終わった)
  瞬間に`Effect.Play()`(内部で`Stop()`→新規`Play()`)を呼んで再始動する設計になっている。
  ユーザー報告の「一定周期で一瞬・部分的に四角形ポリゴンになる」症状は、この再始動の
  タイミングと一致すると推測した。
- 調査した上で除外した仮説: Effekseerの`Handle`(=`int`)がハンドル使い回し
  (エイリアシング)されており、別の再生中エフェクトのハンドルと衝突しているのでは、
  という仮説を検討したが、`Effekseer.Manager.cpp`の`AddDrawSet()`を確認したところ
  `Handle Temp = m_NextHandle; m_NextHandle++;`という単調増加カウンタで、
  (`INT32_MAX`到達時のみ0へラップする以外は)使い回しは起きないと確認できたため除外した。
- 採用した仮説: 生成直後のEffekseerインスタンスは、ビルボードの向き等
  「前フレームとの差分に依存する項目」がまだ計算されておらず、素の(未加工の)四角形に
  近い見た目で1フレームだけ描画されてしまう可能性が高い。多くのパーティクル
  エンジンで見られる一般的な挙動で、「周期的・瞬間的・IsLoop=trueの持続エフェクトで
  発生」という報告内容と整合する。ただし実機の映像による確定はできていない
  （このサンドボック環境には画面を目視する手段が無いため）。
- 対応: `graphics::EffectObject::SetRenderingVisible(bool)`を新設（既存の
  `SetVisible`は`SetShown`+`SetPaused`の両方を切り替えるのに対し、こちらは
  `SetShown`のみを切り替え、内部シミュレーションは止めない）。`ecs::EffectComponent`へ
  `IsHiddenAfterLoopRestart`を追加し、`EffekseerManager::Update()`でループ再始動の
  瞬間に1フレームだけ`SetRenderingVisible(false)`で隠し、次のUpdateで
  `IsPlaying()==true`になった(=内部状態が1tick進んだ)時点で表示を戻すようにした。
- **重要**: この修正は静的解析による仮説ベースの対策であり、実機での確定原因ではない。
  ユーザーに実際のプレイでの症状解消を確認してもらう必要がある。解消しない場合は、
  画面録画等の追加の手がかりを元に別の原因(GPU側のリングバッファ/フレームバッファ
  タイミング、Effekseerのテクスチャロード周りなど)を再調査する。

**追記(2026-07-17)**: ユーザーが実機プレイで症状の解消を確認。上記の仮説(ループ再始動
直後のインスタンスの1フレーム目が素の四角形に近い見た目になる)が実際の原因だったと
確定した。
